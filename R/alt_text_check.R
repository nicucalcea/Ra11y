...
alt_text_check <- function(plot_name, alt_text_loc = c("console"), save_filepath) {
  ...
  if (alt_text == "") {
    ...
    # Write out alt text to console
    if ("console" %in% alt_text_loc) {
      cli::cli_alert_info(alt_text)
    }

    # Save alt text to file
    if ("file" %in% alt_text_loc) {
    ...
    # Save alt text to clipboard
    if ("clipboard" %in% alt_text_loc) {
      clipr::write_clip(alt_text)
    }
  }
